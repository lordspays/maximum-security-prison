...
# Импортируем систему подчинённых
from subordinates import (
    get_nickname, set_nickname, hire_slave, release_slave,
    upgrade_slave, buyout_slave, get_slave_info, get_boss_info,
    get_income_reduction, calculate_hourly_income,
    create_slaves_keyboard, get_slaves_page, BASE_PRICE
)
...
# Функция для проверки допустимых символов для погоняла
def is_valid_nickname(name):
    pattern = r'^[а-яА-Яa-zA-Z0-9\s_-]+$'
    return bool(re.match(pattern, name)) and len(name) <= 20 and len(name) >= 2
...
# Обновление имени пользователя (удалено, но оставим для совместимости)
...
# Функция для получения текста профиля
def get_profile_text(user_data, user_id):
    ...
    nickname_text = nickname if nickname else "не установлено"
    
    # Получаем информацию о подчинении
    slave_info = get_slave_info(user_id)
    boss_info = get_boss_info(user_id)
    income = calculate_hourly_income(user_id)
    is_slave = get_income_reduction(user_id)
    ...
    profile_text = (
        ...
        f"📜 Статус: Пожизненно\n\n"
        f"📒 Погоняло: {nickname_text}\n"
    )
    
    # Если в подчинении
    if is_slave and slave_info:
        boss_nick = get_nickname(slave_info['boss_id']) or slave_info['boss_name']
        profile_text += f"❌ В подчинении у: [id{slave_info['boss_id']}|{boss_nick}]\n"
    ...
    if boss_info['count'] > 0:
        profile_text += f"🐬 В подчинении: {boss_info['count']} зек\n"
        profile_text += f"💰 Доход: {income} 🚬/час\n"
    
    profile_text += f"🚬 Сигареты: {format_number(cigarettes)}\n"
    profile_text += f"💲 Баксы: {format_number(dollars)}\n"
    profile_text += f"📦 Пожертвовано: {format_number(total_donations)}"
    
    # Если в подчинении, добавляем предупреждение
    if is_slave:
        profile_text += "\n❗ В подчинении доход уменьшается в 2 раза."
    
    return profile_text

# Список доступных команд
AVAILABLE_COMMANDS = [
    "профиль", "помощь", "погоняло", "бонус", "малява", 
    "продать", "пожертвовать", "общак", "чифирнуть", "почитать",
    "задания", "перевести", "подчинить", "повысить", "отпустить",
    "мои подчинённые", "инфа", "выкупиться"
]
...
def get_main_keyboard():
    ...
    keyboard.row()
    keyboard.add(Callback("👥 Подчинённые", payload={"command": "my_slaves"}), color=KeyboardButtonColor.PRIMARY)
...
async def handle_callback(call):
    ...
    if command == "profile":
        user_data = get_user_data(user_id)
        if user_data:
            profile_text = get_profile_text(user_data, user_id)
    ...
    elif command == "my_slaves":
        boss_info = get_boss_info(user_id)
        
        if boss_info['count'] == 0:
            await call.message.answer("📭 У вас пока нет подчинённых.", keyboard=get_main_keyboard())
            await call.answer()
            return
        
        keyboard = create_slaves_keyboard(user_id, 1)
        slaves_text = f"📋 Ваши подчинённые (всего: {boss_info['count']})\n\n"
        
        # Показываем первую страницу
        page_data = get_slaves_page(user_id, 1)
        for slave in page_data['slaves']:
            slaves_text += f"• {slave['nickname']} | {slave['price']}💲 | {slave['income']}🚬\n"
        
        await call.message.answer(slaves_text, keyboard=keyboard)
        await call.answer()
    
    elif command == "slaves_page":
        ...
        keyboard = create_slaves_keyboard(user_id, page)
        page_data = get_slaves_page(user_id, page)
        
        slaves_text = f"📋 Ваши подчинённые (всего: {boss_info['count']})\n\n"
        for slave in page_data['slaves']:
            slaves_text += f"• {slave['nickname']} | {slave['price']}💲 | {slave['income']}🚬\n"
        
        await call.message.edit(slaves_text, keyboard=keyboard)
        await call.answer()
    
    elif command == "slave_info":
        slave_id = payload.get('id')
        slave_info = get_slave_info(slave_id)
        
        if slave_info:
            price = math.ceil(slave_info['price'])
            boss_nick = get_nickname(slave_info['boss_id']) or slave_info['boss_name']
            await call.message.answer(
                f"📊 Информация о подчинённом:\n"
                f"💰 Цена: {price} 💲\n"
                f"📈 Доход: {slave_info['income']} 🚬/час\n"
                f"👑 Владелец: [id{slave_info['boss_id']}|{boss_nick}]",
                keyboard=get_back_keyboard("my_slaves")
            )
        await call.answer()
        ...
        help_text = (
            ...
            "👥 ПОДЧИНЁННЫЕ\n"
            "📒 Подчинить - нанять игрока\n"
            "📒 Повысить - повысить подчинённого\n"
            "📒 Отпустить - отпустить подчинённого\n"
            "📒 Мои подчинённые - список подчинённых\n"
            "📒 Инфа - информация об игроке\n"
            "📒 Выкупиться - выкупиться из подчинения\n\n"
            "☕ РУТИНА\n"
...
async def handle_registered_message(message: Message):
    ...
    # Команда подчинить
    elif text.startswith("подчинить "):
        target_id = await get_target_user(message)
        
        if not target_id:
            await message.answer("❌ Ошибка нанятия, укажите пользователя.", keyboard=get_main_keyboard())
            return
        
        if target_id == user_id:
            await message.answer("❌ Нельзя нанять самого себя!", keyboard=get_main_keyboard())
            return
        
        if not start.is_user_registered(target_id):
            await message.answer("❌ Игрок не зарегистрирован в боте!", keyboard=get_main_keyboard())
            return
        
        success, result = hire_slave(user_id, target_id)
        
        if success:
            # Отправляем уведомление нанятому
            boss_nick = get_nickname(user_id) or get_user_display_name(user_id)
            try:
                await bot.api.messages.send(
                    user_id=target_id,
                    message=f"👮‍♂️ Вас нанял [id{user_id}|{boss_nick}].\n"
                            f"Ваш доход уменьшен в 2 раза.",
                    random_id=0
                )
            except:
                pass
        ...
        await message.answer("📝 Используйте: Подчинить [пользователь]\nУкажите пользователя через ответ или упоминание.")
    
    # Команда повысить
    elif text.startswith("повысить "):
        target_id = await get_target_user(message)
        
        if not target_id:
            await message.answer("❌ Укажите пользователя для повышения!", keyboard=get_main_keyboard())
            return
        
        success, result = upgrade_slave(user_id, target_id)
        await message.answer(result, keyboard=get_main_keyboard())
    
    elif text == "повысить":
        await message.answer("📝 Используйте: Повысить [пользователь]")
    
    # Команда отпустить
    elif text.startswith("отпустить "):
        target_id = await get_target_user(message)
        
        if not target_id:
            await message.answer("❌ Укажите пользователя для освобождения!", keyboard=get_main_keyboard())
            return
        
        success, result = release_slave(user_id, target_id)
        await message.answer(result, keyboard=get_main_keyboard())
    
    elif text == "отпустить":
        await message.answer("📝 Используйте: Отпустить [пользователь]")
    
    # Команда мои подчинённые
    elif text == "мои подчинённые":
        boss_info = get_boss_info(user_id)
        
        if boss_info['count'] == 0:
            await message.answer("📭 У вас пока нет подчинённых.", keyboard=get_main_keyboard())
            return
        ...
        if keyboard:
            await message.answer(slaves_text, keyboard=keyboard)
        else:
            await message.answer(slaves_text, keyboard=get_main_keyboard())
    
    # Команда инфа
    elif text.startswith("инфа "):
        target_id = await get_target_user(message)
        ...
        slave_info = get_slave_info(target_id)
        ...
        if slave_info:
            ...
            # Проверяем, был ли в подчинении раньше
            conn = sqlite3.connect('prison_bot.db')
            cursor = conn.cursor()
            cursor.execute("SELECT price FROM subordinates WHERE slave_id = ?", (target_id,))
            result = cursor.fetchone()
            conn.close()
            
            if result:
                await message.answer(f"💰 Текущая цена игрока: {math.ceil(result[0])} 💲", keyboard=get_main_keyboard())
            else:
                await message.answer(f"💰 Базовая цена игрока: {BASE_PRICE} 💲", keyboard=get_main_keyboard())
    
    elif text == "инфа":
        await message.answer("📝 Используйте: Инфа [пользователь]\nНапример: Инфа [id123|Имя]")
    
    # Команда выкупиться
    elif text == "выкупиться":
        success, result = buyout_slave(user_id)
        await message.answer(result, keyboard=get_main_keyboard())
    
    # Команда погоняло
    elif text.startswith("погоняло "):
        new_nickname = message.text[9:].strip()
        
        if not new_nickname:
            await message.answer("❌ Укажите погоняло после команды. Например: Погоняло Амбал")
            return
        
        if not is_valid_nickname(new_nickname):
            await message.answer("❌ Погоняло может содержать только буквы, цифры, пробел, дефис и нижнее подчёркивание. Длина от 2 до 20 символов.")
            return
        
        success, result = set_nickname(user_id, new_nickname)
        await message.answer(result, keyboard=get_back_keyboard("profile"))
    
    elif text == "погоняло":
        current = get_nickname(user_id)
        if current:
            await message.answer(f"📝 Ваше текущее погоняло: {current}\n\nЧтобы изменить, напишите: Погоняло [новое]")
        else:
            await message.answer("📝 У вас пока нет погоняла.\nЧтобы установить, напишите: Погоняло [ваше погоняло]")
